    ...
    def __init__(
        ...
        super().__init__(
            ...
            model=torch.nn.Sequential(
                torch.nn.Linear(in_features=(observation_length + action_length) * history_size, out_features=(observation_length + action_length) * history_size),
                torch.nn.ReLU(),
                torch.nn.Linear(in_features=(observation_length + action_length) * history_size, out_features=(observation_length + action_length) * history_size),
                torch.nn.ReLU(),
                torch.nn.Linear(in_features=(observation_length + action_length) * history_size,out_features=(observation_length + action_length) * history_size),
                torch.nn.ReLU(),
                torch.nn.Linear(in_features=(observation_length + action_length) * history_size, out_features=1),
            ),
